...
import cybergammagroup.ndsmd.utils.GrayscaleControl
import java.util.Locale

class GeneralFeaturesService : BaseBlockingService() {

    companion object {
        const val INTENT_ACTION_REFRESH_ANTI_UNINSTALL = "cybergammagroup.ndsmd.refresh.anti_uninstall"
        const val INTENT_ACTION_REFRESH_GRAYSCALE = "cybergammagroup.ndsmd.refresh.grayscale"
    }
    ...
    private fun traverseNodesForKeywords(
        ...
        if (node.className != null && node.className == "android.widget.TextView") {
            ...
            if (nodeText != null) {
                ...
                if (editTextContent.lowercase(Locale.getDefault()).contains("ndsmd")) {
                    pressHome()
                }
            }
        }
