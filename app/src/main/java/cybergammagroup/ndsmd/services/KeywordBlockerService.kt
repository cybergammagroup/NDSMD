...
import cybergammagroup.ndsmd.blockers.KeywordBlocker
import cybergammagroup.ndsmd.data.blockers.KeywordPacks
...
class KeywordBlockerService : BaseBlockingService() {
    ...
    companion object {
        const val INTENT_ACTION_REFRESH_BLOCKED_KEYWORD_LIST =
            "cybergammagroup.ndsmd.refresh.keywordblocker.blockedwords"

        const val INTENT_ACTION_REFRESH_CONFIG =
            "cybergammagroup.ndsmd.refresh.keywordblocker.config"
    }
    ...
    override fun onAccessibilityEvent(event: AccessibilityEvent?) {
        ...
        if (!isDelayOver(
                lastEventTimeStamp,
                refreshCooldown
            ) || event == null || event.packageName == "cybergammagroup.ndsmd" || KbIgnoredApps.contains(
                event.packageName
            )
        ) {
            return
